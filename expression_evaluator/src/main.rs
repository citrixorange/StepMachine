...
fn main() {
    ...
    let steps:Vec<(StepMachineLabel,Step<Expression<u32>>)>  = vec![
        (ExpressionSteps::Add.into(), |x,y| add(x,y)),
        (ExpressionSteps::Subtract.into(), |x,y| sub(x,y)),
        (ExpressionSteps::Multiply.into(), |x,y| mult(x,y)),
        (ExpressionSteps::Divide.into(), |x,y| div(x,y))
    ];

    let expression: Expression<u32> = Expression::new(Some(5),Some(4),Some(2),Some(2));
    let ptr_expression = Rc::new(RefCell::new(expression));
    let mut step_machine = StepMachine::<Expression<u32>>::new(Some(Rc::clone(&ptr_expression)), None, steps, Some(|x,y,z,w| error_handler(x,y,z,w)));
